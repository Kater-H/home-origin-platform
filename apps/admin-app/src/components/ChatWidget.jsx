...
import { MessageCircle, X, Send, Bot, User, Minimize2, Maximize2, Settings } from 'lucide-react';
...
const ChatWidget = () => {
  ...
  const fetchQuickActions = async () => {
    try {
      const response = await fetch(`${CHATBOT_API_URL}/chat/quick-actions/admin`);
    ...
    try {
      const response = await fetch(`${CHATBOT_API_URL}/chat/message`, {
        ...
        body: JSON.stringify({
          ...
          user_id: 'admin_user_123' // This would come from actual user context
        })
      });
    ...
    if (!isOpen && messages.length === 0) {
      ...
      setTimeout(() => {
        const welcomeMessage = {
          id: Date.now(),
          type: 'bot',
          content: 'Hello! I\'m your admin assistant for Modern Market Connect. ⚙️ I can provide analytics, user management support, and system insights. How can I help you today?',
          timestamp: new Date().toISOString()
        };
      ...
      <div className="bg-purple-600 text-white p-4 rounded-t-lg flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            {!isMinimized && <p className="text-xs text-purple-100">Online • Ready to help</p>}
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <button
              ...
              >
                <div className={`flex items-start space-x-2 max-w-xs ${
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    message.type === 'user' ? 'bg-purple-600 text-white' : 'bg-gray-300 text-gray-600'
                  }`}>
