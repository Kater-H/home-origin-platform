...
const ChatWidget = () => {
  ...
  const fetchQuickActions = async () => {
    try {
      const response = await fetch(`${CHATBOT_API_URL}/chat/quick-actions/buyer`);
    ...
    try {
      const response = await fetch(`${CHATBOT_API_URL}/chat/message`, {
        ...
        body: JSON.stringify({
          ...
          user_id: 'buyer_user_123' // This would come from actual user context
        })
      });
    ...
    if (!isOpen && messages.length === 0) {
      ...
      setTimeout(() => {
        const welcomeMessage = {
          id: Date.now(),
          type: 'bot',
          content: 'Hello! Welcome to Modern Market Connect! 🛒 I\'m here to help you find products, track orders, and answer any questions you have. How can I assist you today?',
          timestamp: new Date().toISOString()
        };
              ...
              >
                <div className={`flex items-start space-x-2 max-w-xs ${
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    message.type === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'
                  }`}>
