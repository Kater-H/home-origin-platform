...
import { MessageCircle, X, Send, Bot, User, Minimize2, Maximize2, Truck } from 'lucide-react';
...
const ChatWidget = () => {
  ...
  const fetchQuickActions = async () => {
    try {
      const response = await fetch(`${CHATBOT_API_URL}/chat/quick-actions/rider`);
    ...
    try {
      const response = await fetch(`${CHATBOT_API_URL}/chat/message`, {
        ...
        body: JSON.stringify({
          ...
          user_id: 'rider_user_123' // This would come from actual user context
        })
      });
    ...
    if (!isOpen && messages.length === 0) {
      ...
      setTimeout(() => {
        const welcomeMessage = {
          id: Date.now(),
          type: 'bot',
          content: 'Hello! I\'m your delivery assistant! 🚴‍♂️ I can help with delivery information, route optimization, earnings, and support. What do you need help with?',
          timestamp: new Date().toISOString()
        };
  ...
  if (!isOpen) {
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <button
          onClick={toggleChat}
          className="bg-orange-600 hover:bg-orange-700 text-white rounded-full p-4 shadow-lg transition-all duration-300 hover:scale-110"
    ...
    <div className={`fixed bottom-6 right-6 z-50 bg-white rounded-lg shadow-2xl border border-gray-200 transition-all duration-300 ${
      ...
      <div className="bg-orange-600 text-white p-4 rounded-t-lg flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <Truck size={20} />
          <div>
            <h3 className="font-semibold">Delivery Assistant</h3>
            {!isMinimized && <p className="text-xs text-orange-100">Online • Ready to help</p>}
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <button
              ...
              >
                <div className={`flex items-start space-x-2 max-w-xs ${
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    message.type === 'user' ? 'bg-orange-600 text-white' : 'bg-gray-300 text-gray-600'
                  }`}>
